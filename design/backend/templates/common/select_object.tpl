...
{elseif $style == "dropdown"}
    <li class="dropdown dropdown-top-menu-item {$class}" {if $select_container_id}id="{$select_container_id}"{/if}>
        <a class="dropdown-toggle cm-combination" data-toggle="dropdown" id="sw_select_{$selected_id}_wrap_{$suffix}" {if $disable_dropdown_processing}data-disable-dropdown-processing="true"{/if}>
            {if $plain_name}
                {$plain_name nofilter}
            {else}
                {if $key_selected}

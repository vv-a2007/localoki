<div id="container_{$container.container_id}" class="container container_{$container.width} {if $container.uses_default_content}container-lock{/if} {if $container.status == "D"}container-off{/if}" {if $container.status != "A"}data-ca-status="disabled"{else}data-ca-status="active"{/if}>

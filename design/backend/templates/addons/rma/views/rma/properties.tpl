...
<div class="table-responsive-wrapper">
    ...
    <tr class="cm-row-status-{$property.status|lower}">
        ...
        <td data-th="{if $smarty.request.property_type == "R"}{__("reason")}{else}{__("action")}{/if}">
...
{capture name="buttons"}
    {capture name="tools_list"}
        {if $properties && $smarty.request.property_type == $smarty.const.RMA_REASON}
            <li class="mobile-hide">{btn type="delete_selected" dispatch="dispatch[rma.m_delete_properties]" form="rma_properties_form"}</li>

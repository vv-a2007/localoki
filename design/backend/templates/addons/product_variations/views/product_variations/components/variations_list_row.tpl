<tr class="multiple-table-row cm-row-status-{if $combination.exists}d{else}a{/if}">
    ...
    <td width="1%">
        <input type="checkbox" name="variation_codes[]" value="{$variation_code}" class="cm-item{if $group_meta} cm-item-{$group_meta}{/if}"{if $combination.exists} checked disabled{/if}/>
    </td>
    <td width="99%">
        {strip}

        ...
        <table class="table table-middle table-last-td-align-right">
            ...
            {foreach from=$orders.0 item="order"}
                <tr>
                    ...
                    <td><a href="{"orders.details?order_id=`$order.order_id`"|fn_url}"><bdi>{__("order")}</bdi> <bdi>#{$order.order_id}</bdi></a> {__("by")} {if $order.user_id}<a href="{"profiles.update?user_id=`$order.user_id`"|fn_url}">{/if}{$order.lastname} {$order.firstname}{if $order.user_id}</a>{/if}</td>

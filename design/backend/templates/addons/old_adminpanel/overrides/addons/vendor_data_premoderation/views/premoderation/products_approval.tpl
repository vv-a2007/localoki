...
<tr>
    ...
    <td>
        ...
        {if $product.approved == "Y" || $product.approved == "P"}
            {include file="common/popupbox.tpl" id="disapprove_`$product.product_id`" text="{__("disapprove")} \"`$product.product|truncate:50:"...":true`\"" content=$smarty.capture.disapprove link_text=" " act="edit" link_class="icon-thumbs-down cm-dialog-auto-size"}
        {/if}
        
        {if $product.approved == "P" || $product.approved == "N"}
            {include file="common/popupbox.tpl" id="approve_`$product.product_id`" text="{__("approve")} \"`$product.product|truncate:50:"...":true`\""  content=$smarty.capture.approve link_text=" " act="edit" link_class="icon-thumbs-up cm-dialog-auto-size"}
        {/if}
...
{if $products}
    ...
    {capture name="buttons"}
        ...
        {include file="buttons/button.tpl" but_role="submit-link" but_target_id="content_disapprove_selected" but_text=__("disapprove_selected") but_meta="cm-process-items cm-dialog-opener cm-dialog-auto-size" but_target_form="manage_products_form"}
    {/capture}
{/if}

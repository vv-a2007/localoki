...
<tr class="cm-row-status-A">
    ...
    {/if}
        <td style="padding-left: {$_shift}px;">
            ...
            <label id="{$title_id}" class="inline-label" for="input_cat_{$cur_cat.category_id}" {if !$cur_cat.has_children && !$cur_cat.subcategories} style="padding-left: 6px;"{/if}>{$cur_cat.name}</label>

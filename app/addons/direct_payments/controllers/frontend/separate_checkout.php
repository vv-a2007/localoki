...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        foreach ($group_products as $vendor_id => $products) {
            ...
            if (md5(serialize($cart['products'])) != $previous_state && empty($cart['skip_notification'])) {
                ...
                if (!empty($added_products)) {
                    ...
                    Tygh::$app['view']->assign('vendor_id', $vendor_id);
                    ...
                    $msg = Tygh::$app['view']->fetch('addons/direct_payments/views/separate_checkout/components/product_notification.tpl');
    ...
    if ($mode == 'shipping_estimation') {
        ...
        Tygh::$app['view']->assign('vendor_id', empty($_REQUEST['vendor_id']) ? '' : $_REQUEST['vendor_id']);
...
if (in_array($mode, array('checkout', 'process_payment'))) {
    $cart = & $cart_service->getCart();

    $payment_methods = fn_prepare_direct_payments_payment_methods($cart, $auth);
...
if ($mode == 'cart') {
    ...
    $group_product_groups = $group_cart_products = array();
    $group_checkout_buttons = $group_take_surcharge_from_vendor = array();
    $group_payment_methods = array();
    $carts_total = 0;
    ...
    foreach ($carts as $vendor_id => &$cart) {
        ...
        if (!fn_cart_is_empty($cart)) {
            list ($cart_products, $product_groups) = fn_calculate_cart_content(
                $cart,
                $auth,
                Registry::get('settings.General.estimate_shipping_cost') == 'Y' ? 'A' : 'S',
                true,
                'F',
                true
            );
            ...
            $group_payment_methods[$vendor_id] = $payment_methods;
        }
    }

    Tygh::$app['view']->assign('group_cart_products', $group_cart_products);
    Tygh::$app['view']->assign('product_groups', $group_product_groups);
    Tygh::$app['view']->assign('group_checkout_add_buttons', $group_checkout_buttons);
    Tygh::$app['view']->assign('group_payment_methods', $group_payment_methods);
    Tygh::$app['view']->assign('carts_total', $carts_total);

    if (fn_allowed_for('MULTIVENDOR')) {
        Tygh::$app['view']->assign('group_take_surcharge_from_vendor', $group_take_surcharge_from_vendor);
    }
...
// All checkout steps
    ...
    Tygh::$app['view']->assign('content_tpl', 'views/checkout/checkout.tpl');
    ...
    Tygh::$app['view']->assign('cart', $cart);
    Tygh::$app['view']->assign('vendor_id', $cart_service->getCurrentVendorId());
    Tygh::$app['view']->assign('cart_products', array_reverse($cart_products, true));
    Tygh::$app['view']->assign('product_groups', $cart['product_groups']);
...
//Purge undeliverable products
    ...
    Tygh::$app['view']->assign('content_tpl', 'views/checkout/complete.tpl');
...
Tygh::$app['view']->assign('cart_is_separate_checkout', true);

// Vendors
$active_carts = $vendor_ids = $vendors = array();
$carts = & $cart_service->getCarts();
...
Tygh::$app['view']->assign('carts', $active_carts);
if (!empty($vendor_ids)) {
    list($vendors) = fn_get_companies(array('company_id' => $vendor_ids), $auth);

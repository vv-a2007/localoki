...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($mode === 'sell_master_product') {
        ...
        $result = $product_manager->createVendorProduct($_REQUEST['master_product_id'],
    ...
    if ($mode === 'm_sell_master_product') {
        ...
        if (!empty($_REQUEST['master_product_ids'])) {
            foreach ($_REQUEST['master_product_ids'] as $master_product_id) {
                $result = $product_manager->createVendorProduct($master_product_id,
                    Registry::get('runtime.company_id'));
                if ($result->isSuccess() && !$result->getData()['vendor_product_exists']) {
...
if ($mode === 'master_products') {
    ...
    $params['merge_with_master_products'] = false;
...
if (!Registry::get('runtime.company_id') && ($mode === 'update' || $mode === 'add' || $mode === 'm_add')) {
    /** @var \Tygh\SmartyEngine\Core $view */
    $view = Tygh::$app['view'];

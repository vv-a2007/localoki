...
//
if ($mode == 'bulk_edit_get_categories_list') {
    if (empty($_REQUEST['bulk_edit_categories_ids'])) {
    ...
    $product_ids = (array) $_REQUEST['bulk_edit_categories_ids'];
    $counter = $category_ids = $data = [];
    $current_company_id = (int) Registry::get('runtime.company_id');
    ...
    foreach ($products as $product) {
        $product_category_ids = (array) $product['category_ids'];

        foreach ($product_category_ids as $category_id) {
            if (!isset($counter[$category_id])) {
                $counter[$category_id] = 0;
            }
    ...
    foreach ($categories as $category) {
        if ($current_company_id && $current_company_id != $category['company_id']) {
            continue;
        }

        $cnt = $counter[$category['category_id']];

        $data[$category['category_id']] = [
            'status' => $cnt == count($product_ids) ? 'A' : 'N'
        ];
    }
    ...
    $view->assign([
        'bulk_edit'          => true,
        'bulk_edit_ids'      => $data,
        'bulk_edit_ids_flat' => array_keys($data)
    ]);
}

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'switch_storefront_status') {
        ...
        if (empty($status) || empty($company_id)) {
            ...
            if ($status === StorefrontStatuses::OPENED) {
...
if ($mode == 'manage') {
    ...
    $params = $_REQUEST;
    if (fn_allowed_for('ULTIMATE')) {
        $params['extend']['storefront_status'] = true;
        $params['extend']['storefront_access_key'] = true;
    }
    ...
    if ($company_id) {
        if (fn_allowed_for('ULTIMATE')) {
            $extra['get_storefront_status'] = true;
            $extra['get_access_key'] = true;
        }

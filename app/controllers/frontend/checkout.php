...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'customer_info') {
        ...
        $cart['recalculate'] = isset($_REQUEST['recalculate_cart'])
            ? $_REQUEST['recalculate_cart'] === 'Y'
            : true;
...
// All checkout steps
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (fn_is_cart_user_data_changed($cart, $auth)) {
            $cart['recalculate'] = true;
        }
    ...
    if (fn_checkout_step_needs_shipping_calculation($cart, $completed_steps)) {
        $cart['calculate_shipping'] = true;
    }

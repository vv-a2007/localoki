...
<div id="est_box{$id_suffix}" align="right">
    ...
    {/if}
        ...
        <input type="hidden" name="result_ids" value="{$shipping_estimation_block_id},shipping_estimation_buttons" />
                    ...
                    {if $group.shippings && !$group.all_edp_free_shipping && !$group.all_free_shipping && !$group.free_shipping && !$group.shipping_no_required}
                        {foreach from=$group.shippings item="shipping" name="estimate_group_shipping"}
                            {if !$show_only_first_shipping || $smarty.foreach.estimate_group_shipping.first}
                                ...
                                <p>
                                    <input type="radio" class="valign" id="sh_{$group_key}_{$shipping.shipping_id}" name="shipping_ids[{$group_key}]" value="{$shipping.shipping_id}" onclick="fn_calculate_total_shipping('{$shipping_estimation_block_id}');" {$checked} /><label for="sh_{$group_key}_{$shipping.shipping_id}" class="valign">{$shipping.shipping} {$delivery_time} - {$rate nofilter}</label>
                                </p>
                                {/hook}
                            {/if}
                        {/foreach}

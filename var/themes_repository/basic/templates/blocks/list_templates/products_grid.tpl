...
{foreach from=$sproducts item="product" name="sproducts"}
    {if $product}
        ...
        <td class="center image-border compact{if !$smarty.foreach.sprod.last && !$show_add_to_cart} border-bottom{/if}" style="width: {$cell_width}%">    
            ...
            <p>{if $item_number == "Y"}{$cur_number}.&nbsp;{math equation="num + 1" num=$cur_number assign="cur_number"}{/if}{assign var="name" value="name_$obj_id"}<bdi>{$smarty.capture.$name nofilter}</bdi></p>
